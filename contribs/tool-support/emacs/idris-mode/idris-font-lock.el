;;; idris-font-lock.el --- Font locking module for Idris Mode
...
;; Purpose:
;;
;; To support fontification of standard Idris keywords, symbols,
;; functions, etc.  Supports full Idris 1.4 as well as LaTeX- and
;; Bird-style literate scripts.
;;
;; Installation:
;; 
;; To turn font locking on for all Idris buffers under the Idris
;; mode of Moss&Thorn, add this to .emacs:
;;
;;    (add-hook 'idris-mode-hook 'turn-on-idris-font-lock)
;;
;; Otherwise, call `turn-on-idris-font-lock'.
;;
;;
;; Customisation:
;;
;; The colours and level of font locking may be customised.  See the
;; documentation on `turn-on-idris-font-lock' for more details.
;;
;;
;; History:
;;
...
;;
;; Version 1.0:
;;   Brought over from Idris mode v1.1.
;;
...
;; `(turn-(on/off)-)idris-font-lock' or `idris-fl-'.
...
(defcustom idris-font-lock-symbols nil
   ...
   (and (fboundp 'decode-char)
	...
	(list (cons "not" (decode-char 'ucs 172))
	      ...
	      (list "." (decode-char 'ucs 8728) ; (decode-char 'ucs 9675)
                    ...
                    ;; forall <foo> . <bar>.
                    'idris-font-lock-dot-is-not-composition)
              (cons "forall" (decode-char 'ucs 8704)))))
  "Alist mapping Idris symbols to chars.
Each element has the form (STRING . CHAR) or (STRING CHAR PREDICATE).
STRING is the Idris symbol.
...
(defun idris-font-lock-dot-is-not-composition (start)
...
(defvar idris-keyword-face 'font-lock-keyword-face)
(defvar idris-constructor-face 'font-lock-type-face)
...
(defvar idris-definition-face 'font-lock-function-name-face)
;; This is probably just wrong, but it used to use
;; `font-lock-function-name-face' with a result that was not consistent with
;; other major modes, so I just exchanged with `idris-definition-face'.
(defvar idris-operator-face 'font-lock-variable-name-face)
(defvar idris-default-face nil)
(defvar idris-literate-comment-face 'font-lock-doc-face
  "Face with which to fontify literate comments.
Set to `default' to avoid fontification of them.")

(defconst idris-emacs21-features (string-match "[[:alpha:]]" "x")
  "Non-nil if we have regexp char classes.
Assume this means we have other useful features from Emacs 21.")

(defun idris-font-lock-compose-symbol (alist)
...
(defun idris-font-lock-symbols-keywords ()
  (when (fboundp 'compose-region)
    (let ((alist nil))
      (dolist (x idris-font-lock-symbols-alist)
	...
	`((,(regexp-opt (mapcar 'car alist) t)
	   (0 (idris-font-lock-compose-symbol ',alist)
...
(defun idris-font-lock-keywords-create (literate)
  "Create fontification definitions for Idris scripts.
Returns keywords suitable for `font-lock-keywords'."
  (let* (;; Bird-style literate scripts start a line of code with
	 ...
	 ;; Most names are borrowed from the lexical syntax of the Idris
	 ;; report.
    ...
    (setq keywords
	  `(;; NOTICE the ordering below is significant
	    ...
	    ,@(unless idris-emacs21-features ;Supports nested comments?
		;; Expensive.
	    ...
	    ,@(idris-font-lock-symbols-keywords)

	    (,reservedid 1 (symbol-value 'idris-keyword-face))
	    (,reservedsym 1 (symbol-value 'idris-operator-face))
            ;; Special case for `as', `hiding', and `qualified', which are
            ;; keywords in import statements but are not otherwise reserved.
            ("\\<import[ \t]+\\(?:\\(qualified\\>\\)[ \t]*\\)?[^ \t\n()]+[ \t]*\\(?:\\(\\<as\\>\\)[ \t]*[^ \t\n()]+[ \t]*\\)?\\(\\<hiding\\>\\)?"
             (1 (symbol-value 'idris-keyword-face) nil lax)
             (2 (symbol-value 'idris-keyword-face) nil lax)
             (3 (symbol-value 'idris-keyword-face) nil lax))

	    ;; Toplevel Declarations.
	    ;; Place them *before* generic id-and-op highlighting.
	    (,topdecl-var  (1 (symbol-value 'idris-definition-face)))
	    (,topdecl-var2 (2 (symbol-value 'idris-definition-face)))
	    (,topdecl-sym  (2 (symbol-value 'idris-definition-face)))
	    (,topdecl-sym2 (1 (symbol-value 'idris-definition-face)))

	    ;; These four are debatable...
	    ("(\\(,*\\|->\\))" 0 (symbol-value 'idris-constructor-face))
	    ("\\[\\]" 0 (symbol-value 'idris-constructor-face))
	    ;; Expensive.
	    (,qvarid 0 (symbol-value 'idris-default-face))
	    (,qconid 0 (symbol-value 'idris-constructor-face))
	    (,(concat "\`" varid "\`") 0 (symbol-value 'idris-operator-face))
	    ;; Expensive.
	    (,conid 0 (symbol-value 'idris-constructor-face))

	    ;; Very expensive.
	    (,sym 0 (if (eq (char-after (match-beginning 0)) ?:)
			idris-constructor-face
		      idris-operator-face))))
    (unless (boundp 'font-lock-syntactic-keywords)
      (case literate
	(bird
	 (setq keywords
	       `(("^[^>\n].*$" 0 idris-comment-face t)
		 ,@keywords
		 ("^>" 0 idris-default-face t))))
	((latex tex)
	 (setq keywords
	       `((idris-fl-latex-comments 0 'font-lock-comment-face t)
		 ,@keywords)))))
    keywords))

;; The next three aren't used in Emacs 21.

(defvar idris-fl-latex-cache-pos nil
  "Position of cache point used by `idris-fl-latex-cache-in-comment'.
Should be at the start of a line.")

(defvar idris-fl-latex-cache-in-comment nil
  "If `idris-fl-latex-cache-pos' is outside a
\\begin{code}..\\end{code} block (and therefore inside a comment),
this variable is set to t, otherwise nil.")

(defun idris-fl-latex-comments (end)
    ...
    (if (= start end)
      ...
      (if (not (eq start idris-fl-latex-cache-pos))
          ...
          (progn
            ...
            ;; Restore position.
            (goto-char start)))
      (if idris-fl-latex-cache-in-comment
          (progn
...
(defconst idris-bird-syntactic-keywords
  (cons '("^[^\n>]"  (0 "<"))
	idris-basic-syntactic-keywords))

(defconst idris-latex-syntactic-keywords
  (append
   '(("^\\\\begin{code}\\(\n\\)" 1 "!")
...
(defcustom idris-font-lock-haddock (boundp 'font-lock-doc-face)
  "If non-nil try to highlight Haddock comments specially."
  :type 'boolean
  :group 'idris)

(defvar idris-font-lock-seen-haddock nil)
(make-variable-buffer-local 'idris-font-lock-seen-haddock)

(defun idris-syntactic-face-function (state)
  "`font-lock-syntactic-face-function' for Idris."
  (cond
   ...
   ((and idris-font-lock-haddock
         (save-excursion
           (goto-char (nth 8 state))
           (or (looking-at "\\(-- \\|{-\\)[|^]")
               (and idris-font-lock-seen-haddock
                    (looking-at "-- ")
                    (let ((doc nil)
                          pos)
                      (while (and (not doc)
                      ...
                      doc)))))
    (set (make-local-variable 'idris-font-lock-seen-haddock) t)
    font-lock-doc-face)
   (t font-lock-comment-face)))

(defconst idris-font-lock-keywords
  (idris-font-lock-keywords-create nil)
  "Font lock definitions for non-literate Idris.")

(defconst idris-font-lock-bird-literate-keywords
  (idris-font-lock-keywords-create 'bird)
  "Font lock definitions for Bird-style literate Idris.")

(defconst idris-font-lock-latex-literate-keywords
  (idris-font-lock-keywords-create 'latex)
  "Font lock definitions for LaTeX-style literate Idris.")

(defun idris-font-lock-choose-keywords ()
  (let ((literate (if (boundp 'idris-literate) idris-literate)))
    (case literate
      (bird idris-font-lock-bird-literate-keywords)
      ((latex tex) idris-font-lock-latex-literate-keywords)
      (t idris-font-lock-keywords))))

(defun idris-font-lock-choose-syntactic-keywords ()
  (let ((literate (if (boundp 'idris-literate) idris-literate)))
    (case literate
      (bird idris-bird-syntactic-keywords)
      ((latex tex) idris-latex-syntactic-keywords)
      (t idris-basic-syntactic-keywords))))

(defun idris-font-lock-defaults-create ()
  "Locally set `font-lock-defaults' for Idris."
...
;; The main functions.
(defun turn-on-idris-font-lock ()
  "Turns on font locking in current buffer for Idris 1.4 scripts.

Changes the current buffer's `font-lock-defaults', and adds the
following variables:

   `idris-keyword-face'      for reserved keywords and syntax,
   `idris-constructor-face'  for data- and type-constructors, class names,
                               and module names,
   `idris-operator-face'     for symbolic and alphanumeric operators,
   `idris-default-face'      for ordinary code.

The variables are initialised to the following font lock default faces:

   `idris-keyword-face'      `font-lock-keyword-face'
   `idris-constructor-face'  `font-lock-type-face'
   `idris-operator-face'     `font-lock-function-name-face'
   `idris-default-face'      <default face>
...
.emacs:
  ...
  (setq font-lock-maximum-decoration '((idris-mode . 2) (t . 0)))

uses level two fontification for `idris-mode' and default level for
...
.emacs:
  ...
  (add-hook 'idris-font-lock-hook
      (lambda ()
          (set-face-foreground 'idris-comment-face \"brown\")))
...
To turn font locking on for all Idris buffers, add this to .emacs:

  (add-hook 'idris-mode-hook 'turn-on-idris-font-lock)

To turn font locking on for the current buffer, call
`turn-on-idris-font-lock'.  To turn font locking off in the current
buffer, call `turn-off-idris-font-lock'.

Bird-style literate Idris scripts are supported: If the value of
`idris-literate-bird-style' (automatically set by the Idris mode
of Moss&Thorn) is non-nil, a Bird-style literate script is assumed.

Invokes `idris-font-lock-hook' if not nil."
  (idris-font-lock-defaults-create)
  (run-hooks 'idris-font-lock-hook)
  (turn-on-font-lock))

(defun turn-off-idris-font-lock ()
  "Turns off font locking in current buffer."
  (font-lock-mode -1))

;; Provide ourselves:

(provide 'idris-font-lock)

;; arch-tag: 89fd122e-8378-4c7f-83a3-1f49a64e458d
;;; idris-font-lock.el ends here
